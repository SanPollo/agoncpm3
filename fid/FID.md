...
```
segment $04   $040000-$042532   Supervisor (cpm3.bin)
...
The heap is whatever is left of segment `$04` after the supervisor, and the CCP buffer. Modules are loaded into it end to end, and a module may take more of it for its own use with `svc_alloc`.
...
The loader reads `FID.INI` from the directory `cpm3.bin` was started in. One filename per line; blank lines and lines starting `;` or `#` are ignored; load order is the order in the file. Wildcards, and directories are allowed:
...
Names are reduced to a canonical form and de-duplicated, so a module named explicitly, and matched by a pattern is loaded once. Note, however, that the `FID.INI` has a maximum file size of 512 bytes.

For each module the loader checks the signature, the length, the checksum and the API version, applies the fixups, zeroes the bss, then calls the module's entry point. On return:

- `A` = 0 — stay resident
- `A` ≠ 0 — discard
- `HL` = a null-terminated sign-on message, or 0 for silence

Every failure is reported, and skipped, as one bad driver must not stop
the others from loading, or the system from booting.

### Heap Ordering

`fid_ptr` is advanced to just past the module before the entry routine runs, and rewound to the module's base if the module declines.
